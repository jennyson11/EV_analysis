...
ORDER BY [전기차_등록대수] DESC; """, language='sql')
    with col1_ins:
        ...
        단순히 등록 대수가 많은 곳이 가장 혼잡한 것이 아니라, 인천이나 대전처럼 '등록 차량 증가 속도를 충전소 구축 속도가 따라가지 못하는 광역시'에서 심각한 병목이 발생하고 있다. 
        지역적 특수성 반영: 강원이나 제주(22.95대)의 경우 거주 인구 대비 충전소가 많은 것은 '관광지'라는 특성이 반영되어 지자체 차원에서 선제적으로 인프라를 확충했을 가능성이 높다. 
        반면 인천은 물류 및 출퇴근용 차량의 급증을 인프라가 수용하지 못하는 병목 현상으로 해석된다.")
...
ORDER BY 단지내_충전기1개당_차량수 DESC; """, language='sql')
    with col2_ins:
        st.markdown("### 💡 분석 인사이트")
        st.warning("거주지 ↔ 공용 인프라의 불균형: 전체 충전소 기준으로는 여유로웠던 제주가 단지 내 기준으로는 전국 최악의 부족 사태를 겪고 있다. 
        이는 제주의 인프라가 관광지나 렌터카 위주의 '공용 인프라'에만 편중되어 도민들의 실거주지 충전 환경을 방치했음을 보여준다. 
        반면 서울·경기는 전체 인프라 대비 주거지 내 충전기 보급 방어율이 매우 뛰어나다. 향후 정책은 거주지(단지 내) 인프라가 부족한 특정 지자체를 타겟으로 한 핀셋 보조금 지원으로 전환되어야 한다.")
...
""", language='sql')
    with col3_ins:
        st.markdown("### 💡 분석 인사이트")
        st.success("보조금 정책의 효과와 할당의 미스매치: 보조금 정책 자체는 전기차 수요를 견인하는 데 확실한 효과를 내고 있으나, 지역별 예산 배정 수요 예측에는 실패했다. 
        구매 의사가 높은 지방(경북, 전북, 제주 등)은 예산 조기 소진으로 전기차 확산에 제동이 걸리는 반면, 인구가 많은 수도권(서울, 경기)은 오히려 예산이 남아돌며 정체되고 있다. 
        따라서 연초에 고정된 물량을 할당하는 방식에서 벗어나, 분기별 실제 소진 속도에 맞춰 수도권의 남는 예산을 지방으로 이관하는 탄력적인 예산 재배치 시스템이 필요하다.")
