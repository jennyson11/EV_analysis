...
""", language='sql')
    with col1_ins:
        st.markdown("### 💡 분석 인사이트")
        # 따옴표 3개로 감싸서 줄바꿈 오류 해결
        st.info("""
        **불균형적인 인프라 공급:** 전기차 보급량 대비 충전 인프라는 전국적으로 매우 불균형하게 공급되고 있다. 
        단순히 등록 대수가 많은 곳이 가장 혼잡한 것이 아니라, 인천이나 대전처럼 '등록 차량 증가 속도를 충전소 구축 속도가 따라가지 못하는 광역시'에서 심각한 병목이 발생하고 있다. 
        
        **지역적 특수성 반영:** 강원이나 제주의 경우 관광지라는 특성이 반영되어 선제적으로 인프라를 확충했을 가능성이 높다. 
        반면 인천은 물류 및 출퇴근용 차량의 급증을 인프라가 수용하지 못하는 현상으로 해석된다.
        """)
...
ORDER BY 단지내_충전기1개당_차량수 DESC; """, language='sql')
    with col2_ins:
        st.markdown("### 💡 분석 인사이트")
        st.warning("""
        **거주지 ↔ 공용 인프라의 불균형:** 전체 충전소 기준으로는 여유로웠던 제주가 단지 내 기준으로는 부족 사태를 겪고 있다. 
        이는 제주의 인프라가 관광지 위주의 '공용 인프라'에 편중되어 도민들의 실거주지 충전 환경을 방치했음을 보여준다. 
        
        **정책 방향:** 서울·경기는 주거지 내 충전기 보급 방어율이 뛰어나다. 향후 정책은 거주지 인프라가 부족한 특정 지자체를 타겟으로 한 핀셋 지원으로 전환되어야 한다.
        """)
...
""", language='sql')
    with col3_ins:
        st.markdown("### 💡 분석 인사이트")
        st.success("""
        **보조금 할당의 미스매치:** 보조금 정책은 전기차 수요를 견인하고 있으나, 지역별 예산 배정 수요 예측에는 실패했다. 
        구매 의사가 높은 지방은 예산 조기 소진으로 제동이 걸리는 반면, 수도권은 예산이 남는 현상이 발생한다. 
        
        **해결책:** 분기별 실제 소진 속도에 맞춰 수도권의 남는 예산을 지방으로 이관하는 탄력적인 예산 재배치 시스템이 필요하다.
        """)
